...
struct ImportFormatter {
    ...
    func formatImports(in file: String) -> SourceFileSyntax {
        ...
        var items = classifyItem(in: sourceFile)
        ...
        items[..<pivotPoint]
            .sort { lhs, rhs in
                guard case let Item.import(lhsImport, _) = lhs,
                   case let Item.import(rhsImport, _) = rhs else {
                    fatalError()
                }
    ...
    func classifyItem(in file: SourceFileSyntax) -> [Item] {
        file.statements
            .map { statement in
